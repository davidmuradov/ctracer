...
int main(int argc, char *argv[]) {
    ...
    light = lights_new_point_light(tuple_new_point(0, 0, -1), tuple_new_color(1, 1, 1));
    in_shadow = 1;
    mat = materials_new_material();
    struct tuple point = tuple_new_point(0, 0, 0);
    struct tuple result = lights_lighting(mat, point, light, eyev, normalv, in_shadow);
    ...
    in_shadow = world_is_shadowed(w, 0, tuple_new_point(0, 10, 0));
    assert(!in_shadow);
    in_shadow = world_is_shadowed(w, 0, tuple_new_point(10, -10, 10));
    assert(in_shadow);
    in_shadow = world_is_shadowed(w, 0, tuple_new_point(-20, 20, -20));
    assert(!in_shadow);
    in_shadow = world_is_shadowed(w, 0, tuple_new_point(-2, 2, -2));
    assert(!in_shadow);
    world_free_world(w);
    ...
    world_add_sphere(w, &s1);
    s2 = sphere_new_sphere(tuple_new_point(0, 0, 0), 1);
    struct matrix4 m_translate = matrix_new_translation4(0, 0, 10);
    sphere_set_transform(&s2, m_translate);
    world_add_sphere(w, &s2);
    r = ray_new_ray(tuple_new_point(0, 0, 5), tuple_new_vector(0, 0, 1));
    inter = intersection_new_intersection(4, &s2);
    comps = intersection_prepare_computations(&inter, &r);
    c = world_shade_hit(w, &comps);
    assert(tuple_equals(c, tuple_new_color(0.1, 0.1, 0.1)));
    world_free_world(w);
